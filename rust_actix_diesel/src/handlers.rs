...
use crate::{
    models::{NewTweet, Tweet, TweetPayload},
    DbPool,
};
...
fn find_by_id(tweet_id: i32, conn: &mut PgConnection) -> Result<Tweet, DbError> {
    use crate::schema::tweets::dsl::*;

    let tweet = tweets.filter(id.eq(tweet_id)).first::<Tweet>(conn)?;
